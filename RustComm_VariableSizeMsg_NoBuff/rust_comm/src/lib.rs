...
{    
        ...
        L: Logger + Copy + Clone + Default
    {
        ...
        let mut strm1 = stream.try_clone()?;
        let mut strm2 = stream.try_clone()?;
        ...
        let _ = std::thread::Builder::new()
            ...
            loop {
                ...
                let rslt = P::recv_message(&mut strm2);
                if rslt.is_err() {
...
pub fn handle_client(strm: TcpStream) -> Result<()> {
    ...
    let mut writer = strm.try_clone()?;
    let mut reader = strm.try_clone()?;
    // let _ = std::thread::spawn(move || {
        loop {
            L::write("\n  attempting to recv message in client handler");
            // let _ = std::io::stdout().flush();
            let rslt:Result<Message> = P::recv_message(&mut reader);
            ...
            let _ = P::send_message(&msg, &mut writer);
        } 
